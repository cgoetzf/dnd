...
tinymce.ThemeManager.add('modern', function(editor) {
	var self = this, settings = editor.settings, Factory = tinymce.ui.Factory,
		each = tinymce.each, DOM = tinymce.DOM, Rect = tinymce.geom.Rect, Floatcard = tinymce.ui.FloatPanel;
	 ...
	 *
	 * @param {tinymce.ui.Panel} card card to add focus to.
	 */
	...
	function renderInlineUI(args) {
		...
		function reposition() {
			if (card && panel.moveRel && panel.visible() && !panel._fixed) {
		...
		function hide() {
			if (panel) {
				// We require two events as the inline float card based toolbar does not have autohide=true
				panel.hide();
		...
		function render() {
			...
			// Render a plain card inside the inlineToolbarContainer if it's defined
			card = self.card = Factory.create({
		...
		// Remove the card when the editor is removed
		editor.on('remove', function() {
			if (panel) {
				panel.remove();
				card = null;
